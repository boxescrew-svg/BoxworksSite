(() => {
      'use strict';
      const canvas = document.getElementById('bg-honeycomb');
      if (!canvas) return;
      if (matchMedia('(prefers-reduced-motion: reduce)').matches) return;

      const ctx = canvas.getContext('2d');

      // Icona Boxworks — stesso path usato in Homepage e nella pagina di Login
      ...
      // Area privata: MOLTO più delicato dei valori usati in Homepage (REST .04 / PEAK .22)
      const WAVE_SPEED  = 0.00020;
      const REST_ALPHA  = 0.035;
      const PEAK_ALPHA  = 0.10;
      const SCALE_BASE  = 0.55;
      const SCALE_PEAK  = 0.75;
      const WAVE_WIDTH  = 0.14;

      let wavePos = 1.0, waveDir = -1;
      let points = [], cellD = 0, dpr = 1, hidden = false, resizeTimer, W = 0, H = 0;
      ...
      function buildHoneycomb(w, h) {
        const COLS_TARGET = 20; // stessa densità della Homepage
        cellD = w / COLS_TARGET;
      ...
      window.addEventListener('resize', () => {
        ...
        resizeTimer = setTimeout(() => { ctx.setTransform(1, 0, 0, 1, 0, 0); resize(); }, 120);
      }, { passive: true });

      if (window.innerWidth >= 768) { resize(); tick(); }
    })();